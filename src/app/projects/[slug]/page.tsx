      ...
      <div className="ml-0 mr-0 md:ml-[30%] md:mr-[30%] pt-[calc(22vh+112px)] pb-16 px-4 flex flex-col z-10 relative">
        ...
        <div className="w-full aspect-video relative mt-8 rounded-lg overflow-hidden border border-black/10 dark:border-white/[0.15] shadow-sm bg-black z-20">
          {project.video ? (
            project.video.includes('youtube') || project.video.includes('youtu.be') ? (
              <iframe
                src={(function getYouTubeEmbedUrl(url: string) {
                  let videoId = '';
                  try {
        ...
        {project.creator && (
          <div className="mt-4 flex items-center gap-2">
            <span className="text-[14px] text-zinc-500 dark:text-zinc-400">Created by:</span>
            <a 
              href={project.creator.github} 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-[14px] font-medium text-zinc-900 dark:text-zinc-100 hover:underline flex items-center gap-1.5 transition-colors"
            >
